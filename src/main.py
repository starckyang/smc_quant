...
class OrderBlockStrategy(Strategy):
    def init(self):
        ...
        # self.market_structure_cp = self.market_structure_cp
    ...
    # unused functions area

    # def choppy_market_identification(self):
    #     ups = 0
    #     downs = 0
    #     for i in range(1, min(24, self.index)):
    #         if self.data.Close[-i] > self.data.Open[-i]:
    #             ups += 1
    #         if self.data.Open[-i] > self.data.Close[-i]:
    #             downs += 1
    #     if np.abs(ups-downs) > self.market_structure_cp:
    #         self.market_structure = "trendy"
    #     else:
    #         self.market_structure = "choppy"
